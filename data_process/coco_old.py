...
class COCO2014(data.Dataset):
    def __init__(
        ...
        **kwargs 
        ...
        if phase == 'train':
            ...
            # self.img_list, _, self.labels, _ = train_test_split(
            #     self.img_list, self.labels, test_size=.7, random_state=111
            # )
        else:
            self.labels = self.true_labels

        #print(f'Training labels: {self.labels}')

        self.noisy_val = kwargs.get('noisy_val', False)
        n = len(self.true_labels)
        if phase == 'val0' or phase == 'val1':
            # self.img_list = self.img_list[:n//2]
            # self.labels = self.labels[:n//2]
            self.img_list, _, self.labels, _ = train_test_split(
                self.img_list, self.labels, test_size=.5, random_state=111
            )
            if phase == 'val0':
                self.img_list, _, self.labels, _ = train_test_split(
                    self.img_list, self.labels, test_size=.5, random_state=111
                ...
                _, self.img_list, _, self.labels = train_test_split(
                    self.img_list, self.labels, test_size=.5, random_state=111
                )

        #print(f'val0 labels: {self.labels}')
        ...
        if phase == 'test':
            ...
            # self.img_list = self.img_list[n//2:]
            # self.labels = self.labels[n//2:] 
            print('test size', len(self.img_list))

        #print(f'val1 labels: {self.labels}')
       

    def get_anno(self):
        phase_ = self.phase if 'val' not in self.phase else 'val'
        phase_ = phase_ if self.phase != 'test' else 'val'

        list_path = os.path.join(
            self.root, 
            #'data', 
            '{}_anno.json'.format(phase_)
        )
        self.img_list = json.load(open(list_path, 'r'))
        self.cat2idx = json.load(open(os.path.join(
            self.root, 
            #'data', 
            'category.json'
        ), 'r'))
    ...
    def get_true_labels(self):
        ...
        list_path = os.path.join(
            self.root, 
            #'data', 
            '{}_anno2.json'.format(phase_)
        )
        labels = json.load(open(list_path, 'r'))
        true_labels = np.zeros((len(labels),len(self.cat2idx))) 
        for i, label in enumerate(labels):
            true_labels[i, label] = 1
        print('true labels from get_true_labels', true_labels)
        return true_labels
    ...
    def __getitem__(self, index):
        ...
        return {'data': self.get(item), 'labels': target}
    ...
    def get(self, item):
        ...
        img = Image.open(os.path.join(self.root, '{}2014'.format(phase_), filename)).convert('RGB')
